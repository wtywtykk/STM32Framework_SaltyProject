...
static int8_t USBCDC_Init(void);
static int8_t USBCDC_DeInit(void);
static int8_t USBCDC_Control(uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t USBCDC_Receive(uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_USBCDC_fops =
{
  USBCDC_Init,
  USBCDC_DeInit,
  USBCDC_Control,
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t USBCDC_Receive(uint8_t* Buf, uint32_t *Len)
{
...
int VCP_write(const void *pBuffer, int size)
{
	...
	if (pCDC)
	{
		...
		while (pCDC->TxState)
		{
			...
			{
				//The connection may be broken,restart usb
				USB_Restart();
				return 0;
			}
...
int VCP_getlen(void)
{
	return s_RxBuffer.Size - s_RxBuffer.Position;
}